...
export default function CalendarPage() {
  ...
  const subsRes = useResource<{ items: Subscription[] }>(
    ...
    <div>
          ...
          <div className="flex items-center gap-2">
            <div className="hidden sm:inline-flex items-center gap-1 rounded-md border border-line bg-sunken/70 p-0.5">
              <button
                ...
                className={`rounded-[5px] px-2 py-1 text-xs font-medium transition-colors ${viewMode === "grid" ? "bg-surface text-white" : "text-ink3 hover:text-white"}`}
              >
                Grid
              </button>
              <button
                ...
                className={`rounded-[5px] px-2 py-1 text-xs font-medium transition-colors ${viewMode === "list" ? "bg-surface text-white" : "text-ink3 hover:text-white"}`}
              >
                List
              </button>
            </div>
        ...
        <div className="rounded-2xl border border-line bg-surface p-4 sm:p-6">
          {viewMode === "grid" ? (
                      ...
                      <ul className="mt-1 space-y-0.5">
                        {dayEvents.slice(0, 3).map((evt, j) => (
                          <li key={j} className="group relative">
                            <span
                              className={`block truncate rounded px-1 py-0.5 text-[10px] font-medium text-white ${evt.color}`}
                            >
                              {evt.label}
                            </span>
                        ...
                        {dayEvents.length > 3 && (
                          <li className="px-1 text-[10px] text-ink3">
            ...
            <div className="space-y-2">
                  ...
                  return (
                    ...
                    >
                      <div className="mb-2 flex items-center gap-2">
                        <span
                          className={`inline-flex size-6 items-center justify-center rounded-full text-[11px] font-semibold ${isToday ? "bg-brand text-white" : "text-ink2"}`}
                        >
                          {day}
                        </span>
                      ...
                      <ul className="space-y-1">
                        {dayEvents.map((evt, j) => (
                          ...
                          >
                            <span
                              className={`inline-block rounded px-1.5 py-0.5 text-xs font-medium text-white ${evt.color}`}
                            >
                              {evt.label}
                            </span>
                            {evt.amount !== undefined && (
                              <span className="text-xs font-medium text-ink3 sm:hidden">
                                {formatMoney(evt.amount, currency, locale)}
                              </span>
                            )}
                          </li>
              ...
              {Array.from({ length: daysInMonth }, (_, i) => i + 1).every(
                (d) => !events.has(d),
              ) && (
      ...
      <div className="mt-4 flex flex-wrap items-center gap-4 text-xs text-ink3">
        <span className="flex items-center gap-1.5">
          <span className="h-2.5 w-2.5 rounded-full bg-neg" /> Bill
        </span>
        <span className="flex items-center gap-1.5">
          <span className="h-2.5 w-2.5 rounded-full bg-info" /> Subscription
        </span>
        <span className="flex items-center gap-1.5">
          <span className="h-2.5 w-2.5 rounded-full bg-warn" /> Recurring
          Expense
        </span>
        <span className="flex items-center gap-1.5">
          <span className="h-2.5 w-2.5 rounded-full bg-pos" /> Recurring Income
        </span>
      </div>
    </div>
  );
