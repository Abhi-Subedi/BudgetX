...
export function SpendingChart({ series, currency, locale = "en-US" }: Props) {
  ...
  const geometry = useMemo(() => {
    ...
    const maxVal = Math.max(
      ...series.map((p) => Math.max(p.current, p.previous)),
      100,
    );
    ...
    const prevPts = series.map((s, i) =>
      toXY(Math.min(s.previous, niceMax), i),
    );
  ...
  const linePrev = buildPath(
    prevPts.slice(0, Math.max(1, Math.min(prevPts.length, currentPts.length))),
  );
    ...
    <div className="relative">
      ...
      >
        ...
        {activePoint && activeIdx !== null ? (
          <g>
            <line
              x1={currentPts[activeIdx].x}
              x2={currentPts[activeIdx].x}
              y1={PAD_TOP}
              y2={baseline}
              stroke="#10B981"
              strokeWidth="1"
            ...
            />
            <circle
              cx={currentPts[activeIdx].x}
              cy={currentPts[activeIdx].y}
              r="4.5"
              fill="#10B981"
              stroke="#0B1120"
              strokeWidth="2"
      ...
      {activePoint ? (
        <div className="pointer-events-none absolute left-1/2 top-0 -translate-x-1/2 rounded-md border border-line bg-sunken px-3 py-1.5 text-xs text-white shadow-lift">
          <span className="tnum font-semibold">Day {activePoint.day}</span>
          <span className="mx-1.5 text-white/40">·</span>
          <span className="tnum">
            {formatMoney(activePoint.current, currency, locale)}
          </span>
        </div>
      ) : null}
    </div>
