      ...
      {options.map((opt) => {
        ...
        return (
          <button
            ...
            } ${active ? "bg-line text-paper shadow-line" : "text-ink3 hover:text-ink2"}`}
          >
            {opt.label}
          </button>
        );
...
export function ProgressBar({
  ...
  const resolved = tone ?? (clamped >= 100 ? "neg" : clamped >= 75 ? "warn" : "brand");
  const color = resolved === "neg" ? "bg-neg" : resolved === "warn" ? "bg-warn" : "bg-brand";
  ...
  return (
    <div className={`w-full overflow-hidden rounded-full bg-sunken ${height} ${className}`} role="presentation">
      <div
...
export function Avatar({ name, className = "" }: { name: string; className?: string }) {
  const initials = name
    ...
    .join("");
  const palette = ["bg-brand-tint text-brand-strong", "bg-warntint text-warn", "bg-negtint text-neg", "bg-sunken text-ink2"];
  let hash = 0;
