...
def list_accounts(
    ...
    accounts = account_service.list_accounts(db, user.id, include_archived=include_archived)
    return {
...
def delete_account(account_id: int, user: CurrentUser, db: DbSession):
    account_service.delete_account(db, user.id, account_id)


@router.get("/categories")
def list_categories(user: CurrentUser, db: DbSession, kind: CategoryKind | None = None):
    categories = category_service.list_categories(db, user.id, kind)
    return {
        "items": [
            {
                "id": c.id,
